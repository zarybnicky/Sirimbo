...
CREATE TYPE public.event_capacity_unit AS ENUM ('people', 'registrations');
...
CREATE TYPE public.event_registration_source AS ENUM ('self', 'manager', 'cohort');
...
CREATE TABLE public.event_instance_registration (
  ...
  person_id bigint REFERENCES public.person (id)
    ON UPDATE CASCADE
    ON DELETE CASCADE,
  target_cohort_id bigint REFERENCES public.cohort (id)
    ON UPDATE CASCADE
    ON DELETE RESTRICT,
...
CREATE TABLE public.event_lesson_demand (
  ...
  tenant_id bigint DEFAULT public.current_tenant_id() NOT NULL REFERENCES public.tenant (id)
    ON UPDATE CASCADE
    ON DELETE CASCADE,
  trainer_id bigint NOT NULL REFERENCES public.event_instance_trainer (id)
    ON UPDATE CASCADE
    ON DELETE CASCADE,
...
CREATE TYPE public.announcement_type_input AS (id bigint, title text, body text, is_sticky boolean, scheduled_since timestamp with time zone, scheduled_until timestamp with time zone, status public.announcement_status);

CREATE TYPE public.article_type_input AS (id bigint, title text, preview text, body text, title_photo_url text, is_visible boolean);
...
CREATE TYPE public.series_info AS (id bigint, name text, "position" int, length int, since timestamp with time zone, until timestamp with time zone);
