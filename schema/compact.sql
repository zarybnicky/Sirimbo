...
CREATE TABLE csts.athlete_ranking (
  athlete_id int NOT NULL REFERENCES csts.athlete (idt)
    ON DELETE CASCADE,
  discipline text NOT NULL,
  series text NOT NULL,
  personal_class text,
  personal_points int,
  personal_domestic_finale_count int,
  personal_foreign_finale_count int,
...
CREATE TABLE csts.couple (
  ...
  couple_idt int NOT NULL UNIQUE,
  man_idt int NOT NULL REFERENCES csts.athlete (idt)
    ON DELETE CASCADE,
  woman_idt int NOT NULL REFERENCES csts.athlete (idt)
    ON DELETE CASCADE,
...
CREATE TABLE csts.competitor_ranking (
  ...
  class text,
  points int,
  domestic_finale_count int,
  foreign_finale_count int,
  ranklist_ranking int,
  ranklist_points int,
  athlete_idt int REFERENCES csts.athlete (idt)
    ON DELETE CASCADE,
  couple_id int REFERENCES csts.couple (id)
    ON DELETE CASCADE,
  CHECK (
    (athlete_idt IS NOT NULL
      AND couple_id IS NULL)
      OR (athlete_idt IS NULL
