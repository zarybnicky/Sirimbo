import { startOf, getTotalMin, getMinutesFromMidnight, getSlotDate, getDstOffset, add, lt, gt, merge, min, inRange, diff, max, eq } from "../localizer"

const getKey = ({ start, end, step, timeslots }) =>
      `${+startOf(start, 'minutes')}-${+startOf(end, 'minutes')}-${step}-${timeslots}`
