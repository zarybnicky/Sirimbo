...
import type { DaySlotMetrics } from './utils/DaySlotMetrics'

let isSegmentInSlot = (seg, slot) => seg.left <= slot && seg.right >= slot
let eventsInSlot = (segments, s) => segments.filter((seg) => isSegmentInSlot(seg, s)).length
...
const EventEndingRow: React.FC<{
  ...
  onDoubleClick: () => void;
  onKeyPress: () => void;
  onShowMore: (slot: number, e: EventTarget & HTMLButtonElement) => void;
}> = ({
  className,
  segments = [],
  selected = {},
  ...
  let current = 1,
    ...
    while (current <= slots) {
      ...
      if (range(left, left + span).every((s) => eventsInSlot(segments, s) === 1)) {
        if (gap) {
          row.push(
        ...
        if (gap) {
          row.push(<div key={`${key}_gap`} className="rbc-row-segment" style={{ flexBasis: `${(Math.abs(gap) / slots) * 100}%` }} />);
        }
            ...
            {!count ? null : (
              <button
                ...
                onClick={(e) => {
                  e.preventDefault()
                  e.stopPropagation()
                  onShowMore(current, e.currentTarget)
                }}
              >
                {`+${count} dalších`}
              </button>
