...
class WeekWrapper extends React.Component {
  ...
  _selectable = () => {
    ...
    selector.on('selecting', (box) => {
      ...
      if (action === 'resize') this.handleResize(box, bounds)
    })
    ...
    selector.on('select', (point) => {
      ...
      if (pointInBox(getBoundsForNode(node), point)) {
        const { resourceId, isAllDay } = this.props
        const { event } = this.state.segment
        this.context.draggable.onEnd({start: event.start, end: event.end, resourceId, isAllDay})
      }
      this.setState({ segment: null })
    })
