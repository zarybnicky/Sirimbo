...
- Don't hand-write files under `frontend/graphql/`; add/edit documents in the root `graphql/` folder and run `pnpm schema` (or `pnpm schema-starlet` for the starlet tenant bundle) to regenerate. This only needs the local `schema.graphql`/`frontend/starlet/schema.graphql` dumps, not a live DB, so it works offline. Commit the regenerated `frontend/graphql/*` diff together with the `.graphql` source change—that's the existing convention in this repo's history (see e.g. commit `860cea6`)—unless the task says otherwise. A change to a widely-spread fragment (e.g. `EventWithTrainer`) will ripple into unrelated-looking generated files (`ActivityTimeline.ts`, `EventConflicts.ts`, ...); that's expected, review the diff rather than reverting it.
- `pnpm install`/`pnpm <script>` warn `Unsupported engine: wanted: {"node":"24.x"}` when run under Node 22.x—harmless, the commands still work.

## Event data model quick reference
- `event_instance` is self-referential via `parent_id` (FK to its own `id`) and separately can belong to an `event_series` via `series_id`—these are different groupings: `parent_id` nests one instance under another (e.g. `LESSON`/`RESERVATION` rows scheduled under a `CAMP`, exposed via `childEventInstancesList`/`eventInstancesForRangeList(parentId: ...)`), while `series_id` links recurring instances of the same recurring event (exposed as `seriesInfo { position, length, ... }`). PostGraphile exposes the self-FK as a `parent: EventInstance` field.
- `app_private.can_trainer_edit_instance(id)` walks the `parent_id` chain upward to decide trainer access, so a trainer managing a child instance can usually also see/edit its ancestors—but not always (a `LESSON` trainer isn't necessarily on the parent `CAMP`'s trainer list), so treat `parent`/ancestor fields as possibly-null even for trainers.
- The two user-facing "event detail" surfaces are `frontend/ui/EventSummary.tsx` (popover/dialog content shown from `frontend/ui/EventButton.tsx` in the calendar) and `frontend/ui/BasicEventInfo.tsx` (the "Info" tab of the standalone `/termin/[id]` page, `frontend/app/(member)/termin/[id]/EventPageClient.tsx`). Both consume overlapping GraphQL fragments from `graphql/Event.graphql` (`EventWithTrainer` extended by `EventInstancePage`)—add shared fields to `EventWithTrainer` so both surfaces pick them up. `BasicEventInfo` additionally renders a `shared`/anonymous variant (`ISharedEventInstanceResult`, from the raw SQL in `termin.sql`/`termin.queries.ts`) for share-token/public-link views, which does not carry the same fragment fields—guard new fields with `!shared` or extend the SQL query if the public view needs them too.
