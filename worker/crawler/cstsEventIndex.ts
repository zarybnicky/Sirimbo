  ...
  eventCompetitions: z
    .array(
      z.object({
        ...
        competitionsStart: z.iso.time().optional(),
        juryMeeting: z.iso.datetime({ offset: true }).optional(),
...
export const cstsEventIndex: JsonLoader<Response> = {
  ...
  async load(client, parsed) {
    ...
    const eventLocations = await resolveEventVenueLocations(parsed.collection.map((e) => {
      const { city, street, gps, zipCode } = e.eventCompetitions.find((x) => x)!;
      return {
        country: 'Czechia',
        city,
    ...
    return {
      upsertFrontier: parsed.collection.map((e) => ({
        federation: 'csts',
        kind: 'event',
        key: e.id.toString(),
      })),
