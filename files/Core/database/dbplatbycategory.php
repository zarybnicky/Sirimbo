...
class DBPlatbyCategory extends Database
{
    ...
    {
        list($name, $symbol, $amount, $dueDate, $validFrom, $validTo, $useBase, $usePrefix, $archive, $visible)
            = self::escape($name, $symbol, $amount, $dueDate, $validFrom, $validTo, $useBase, $usePrefix, $archive, $visible);
        self::query(
            "INSERT INTO platby_category
             ...
             VALUES
                 ('$name',$symbol,'$amount','$dueDate','$validFrom',
                  '$validTo','$useBase','$usePrefix','$archive','$visible')"
        );
    }

    public static function update($id, $name, $symbol, $amount, $dueDate, $validFrom, $validTo, $useBase, $usePrefix, $archive, $visible)
    {
        list($id, $name, $symbol, $amount, $dueDate, $validFrom, $validTo, $useBase, $usePrefix, $archive, $visible)
            = self::escape($id, $name, $symbol, $amount, $dueDate, $validFrom, $validTo, $useBase, $usePrefix, $archive, $visible);
        self::query(
            "UPDATE platby_category SET
                 pc_name='$name',pc_symbol=$symbol,pc_amount='$amount',
                 pc_date_due='$dueDate',pc_valid_from='$validFrom',
                 pc_valid_to='$validTo',pc_use_base='$useBase',
                 pc_use_prefix='$usePrefix',pc_archive='$archive',
                 pc_visible='$visible'
             WHERE pc_id='$id'"
        );
    }
        ...
        $res = self::query(
            "SELECT * FROM platby_category
             WHERE NOT EXISTS (
                 ...
                 WHERE pcg_id_category=pc_id AND pcg_id_group='$id'
             )"
        );
        return self::getArray($res);
    }
