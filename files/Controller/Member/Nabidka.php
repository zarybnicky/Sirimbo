...
class Controller_Member_Nabidka extends Controller_Member {
	...
	function view($id = null) {
			...
			foreach($items as &$row) {
				$new_data = array(
						...
						'canDelete' => !$data['n_lock'] && Permissions::check('nabidka', P_MEMBER) &&
							($row['p_id'] === User::getParID() || Permissions::check('nabidka', P_OWNED, $data['n_trener'])),
		...
		$this->render('files/View/Member/Nabidka/Overview.inc', array(
				'data' => $nabidky
		));
	}
	...
	private function __processPost() {
		if(empty($_POST))
			return;
		$data = DBNabidka::getSingleNabidka(post('id'));
		
		if(is_object($f = $this->__checkData($data))) {
