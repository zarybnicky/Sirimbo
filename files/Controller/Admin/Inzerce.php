...
class Controller_Admin_Inzerce extends Controller_Admin {
	function __construct() {
		Permissions::checkError('inzerce', P_OWNED);
	}
	function view($id = null) {
		if(empty($_POST)) {
			$this->render("files/Admin/Inzerce/Display.inc");
			return;
		}
		...
		switch(post("action")) {
			...
			case 'edit':
				...
				if($inzerce[0])
					$this->redirect('/admin/inzerce/edit/' . $inzerce[0]);
				break;
			case 'remove':
	...
	function add($id = null) {
		if(empty($_POST) || is_object($f = $this->checkData($_POST, 'add'))) {
			$this->render('files/Admin/Inzerce/Form.inc');
			return;
		}
		...
		DBInzerce::addInzerat(post('kat'), post('reg'), $jmeno, $prijmeni,
			...
			'', (bool) post('visible'), '1');
		$this->redirect('/admin/inzerce', 'Inzerát přidán');
		
	}
	function edit($id = null) {
		if(!$id || !($data = DBInzerce::getSingleInzerat($id)))
			$this->redirect('/admin/inzerce', 'Inzerát s takovým ID neexistuje');
		...
		$this->redirect("/admin/inzerce", "Inzerát upraven");
	}
	function remove($id = null) {
		if(empty($_POST) || post('action') !== 'confirm') {
			$this->render('files/Admin/Inzerce/DisplayRemove.inc');
			return;
		}
		if(!is_array(post('inzerce')))
			$this->redirect('/admin/inzerce');
		foreach(post('inzerce') as $id)
			DBInzerce::removeInzerat($id);
		$this->redirect('/admin/inzerce', 'Inzeráty odebrány');
	}
	function unconfirmed($id = null) {
		if(empty($_POST) || !in_array(post('action'), array('confirm', 'remove'))) {
			$this->render('files/Admin/Inzerce/DisplayNew.inc');
			return;
		}
		if(!is_array(post('inzerce')))
			$this->redirect('/admin/inzerce/unconfirmed', 'Nevybrali jste žádné inzeráty');
		...
		if(post('action') == 'confirm') {
			...
			$this->redirect('/admin/inzerce', 'Inzeráty potvrzeny');
