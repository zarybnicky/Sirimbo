...
import { ChangeCoursesForm } from './_change-courses-form';
import { ChangeFoldersForm } from './_change-folders-form';
...
export default function StarletImportPage() {
  ...
  const [coursesWithStudents, setCoursesWithStudents] = useState<{ course: Course; couples: string[]; solos: string[]; students: Student[] }[]>([]);
  const [studentMap, setStudentMap] = useState<Map<string, (Student & { partner: string | null; })[]>>(new Map());

  useEffect(() => {
    if (!token?.auth_ok) return;

    fetchCoursesWithStudents(token.auth_token, courses.map(x => x[0])).then(([courses, students]) => {
      ...
      <div className="prose">
        ...
        {[...studentMap.entries()].filter(x => x[1].length > 1).map(x => (
          <li key={x[0]}>
            ...
            {' - objeven v '}
            {x[1]
              .map(student => coursesWithStudents.find(course => course.course.key === student.course_key)?.course.code)
              .join(', ')}
            <ul>
...
function diffObjectsList(objects: Record<string, string>[]) {
  ...
  for (const key of keys) {
    if (['key', 'card_id', 'short_id', 'course_key', 'ref_gid', 'ref_key', 'var_sym', 'reg_datetime', 'course_cost', 'paid_amount', 'reg_online', 'reg_by_admin', 'card_out', 'discount', 'comment'].includes(key))
      continue;
...
async function fetchCoursesWithStudents(auth_token: string, courses: string[]) {
  const result: { course: Course; solos: string[]; couples: string[]; students: Student[] }[] = [];
  ...
  for (const course of courses) {
    ...
    for (const student of data?.students || []) {
      ...
      const existingStudents = studentMap.get(name) || studentMap.get(`${name} ${student.year}`) || [];
      ...
      if (partner) {
        ...
        couples.add(student.sex === 'MALE' ? `${name} - ${partnerName}` : `${partnerName} - ${name}`);
      } else {
        solos.add(name);
      }
...
function getNormalizedName(student: { name: string | null; surname: string | null;}): string {
