...
import { MyReservationFragment, useSetDesiredLessonsMutation } from 'lib/graphql/Reservation';
...
export const ReservationButton = ({ item }: { item: MyReservationFragment }) => {
  ...
  const disableAdd = isLoading || freeLessons < 1 || (item.nMaxPocetHod > 0 && myLessons >= item.nMaxPocetHod);

  const addLesson = React.useCallback(() => modifyLessons({
    id: item.id,
    lessonCount: Math.min(myLessons + 1, myLessons + freeLessons),
  }), [item, myLessons]);
  const removeLesson = React.useCallback(() => modifyLessons({
    id: item.id,
        ...
        >
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={removeLesson} disabled={disableRemove}
            >
            ...
            <button
              className="text-red-500 disabled:text-red-200"
              onClick={addLesson} disabled={disableAdd}
            >
