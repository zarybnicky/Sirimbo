...
export class ReservationSelect extends React.Component<{}, {
    ...
    handleOnChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        if (event.target.value == 'none') {
            this.setState({ id: null, data: null });
        }
    ...
    render = () => (
        <div>
            <select className='team-selection' value={this.state.id || 'none'} onChange={this.handleOnChange}>
                ...
                {this.state.options.map(x => <option value={x.key} key={x.value}>{x.value}</option>)}
            </select>
            {this.state.data && <div>
                <div className="trenink-header">
                    ...
                    <div className="date">
                        ...
                        {this.state.data.n_do && this.state.data.n_od !=
                            this.state.data.n_do && this.state.data.n_do}
                    </div>
