CREATE or replace FUNCTION current_person_ids() RETURNS bigint[] LANGUAGE sql STABLE LEAKPROOF PARALLEL SAFE AS $$
  SELECT coalesce(nullif(current_setting('jwt.claims.my_person_ids', true), ''), '{}')::bigint[];
$$;
...
SELECT coalesce(nullif(current_setting('jwt.claims.my_couple_ids', true), ''), '{}')::bigint[];
$$;
...
CREATE or replace FUNCTION my_tenants_array() RETURNS bigint[] LANGUAGE sql STABLE LEAKPROOF PARALLEL SAFE AS $$
  SELECT coalesce(nullif(current_setting('jwt.claims.my_tenant_ids', true), ''), '{}')::bigint[];
$$;
