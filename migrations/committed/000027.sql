--! Previous: sha1:7990d6da739d3b439422e0236dba9504989b23f6
--! Hash: sha1:1626b90d49f49d96087949cf2561b197bc2516ca
...
delete from event_registration where id in (
  select event_registration.id
  from event
  join event_target_cohort on event_id=event.id
  join event_registration on target_cohort_id=event_target_cohort.id
  left join cohort_membership on event_target_cohort.cohort_id=cohort_membership.cohort_id and active_range @> now() and event_registration.person_id=cohort_membership.person_id and event_registration.event_id=event.id
  where event_target_cohort.cohort_id in (23,24) and cohort_membership.id is null
);
...
where cohort_membership.cohort_id in (23,24) and event_registration.id is null;

-- person's current cohorts
-- events these cohorts are registered to
-- -> create

-- person's non-active cohorts
-- events to which this person is registered through them
-- -> make inactive? mark as cancelled? mark attendance as MISSING/EXCUSED
