--! Previous: sha1:6c19531bd8babf55e5762eb7323d559a20cc307a
--! Hash: sha1:7bf5c19ffde28889b1145f1dad3c988710d46104
...
do $$
begin
  if not exists (select * from pg_type where typcategory='E' and typname = 'application_form_status') then
    create type application_form_status as enum (
...
create table membership_application (
  ...
  tenant_id bigint not null DEFAULT public.current_tenant_id() references tenant (id) on update cascade on delete cascade,
    ...
    email public.citext,
    phone text,
    created_by bigint not null references users (u_id) on update cascade on delete cascade,
    status application_form_status not null,
