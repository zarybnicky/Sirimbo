...
class DBNastenka extends Adapter implements Pagable
{
    ...
    public static function addNastenkaSkupina($rodic, $skupina, $color, $popis) {
        list($rodic, $skupina, $color, $popis) =
            self::escapeArray(array($rodic, $skupina, $color, $popis));

        self::query("INSERT INTO upozorneni_skupiny (ups_id_rodic,ups_id_skupina,ups_color,ups_popis)
    ...
    public static function getNastenkaUserName($id) {
        ...
        $res = self::query("SELECT u_login FROM upozorneni LEFT JOIN users ON up_kdo=u_id WHERE " .
            "up_id='$id'");
        if (!$res) {
            return false;
        } else {
    ...
    public static function addNastenka($userid, $nadpis, $text, $lock) {
        list($userid, $nadpis, $text, $lock) =
            self::escapeArray(array($userid, $nadpis, $text, $lock));

        self::query("INSERT INTO upozorneni (up_kdo,up_nadpis,up_text,up_lock,up_timestamp_add) VALUES " .
    ...
    public static function editNastenka($id, $nadpis, $text, $lock) {
        list($id, $nadpis, $text, $lock) =
            self::escapeArray(array($id, $nadpis, $text, $lock));

        self::query("UPDATE upozorneni SET " .
