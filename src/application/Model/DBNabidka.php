...
class DBNabidka extends Adapter
{
    ...
    public static function editNabidka($id, $trener, $pocet_hod, $max_hod, $od, $do, $visible, $lock)
    {
        ...
        self::query(
            "UPDATE nabidka
            SET n_trener='$trener',n_pocet_hod='$pocet_hod',
                n_max_pocet_hod='$max_hod',n_od='$od',
                n_do='$do',n_visible='$visible',n_lock='$lock'
            WHERE n_id='$id'"
        );
    ...
    public static function getNabidkaLessons($parent_id, $u_id)
    {
        ...
        $res = self::query(
            "SELECT ni_pocet_hod
            FROM nabidka_item
            WHERE ni_id_rodic='$parent_id' AND ni_partner='$u_id'"
        );
        if (!$res) {
            return false;
        } else {
    ...
    {
        list($user_id, $parent_id, $pocet_hod) =
            self::escape($user_id, $parent_id, $pocet_hod);

        self::query(
            "INSERT INTO nabidka_item
            (ni_partner,ni_id_rodic,ni_pocet_hod)
    ...
    {
        list($id, $partner, $pocet_hod) =
            self::escape($id, $partner, $pocet_hod);

        $res = self::query(
            "SELECT ni_id,ni_id_rodic
            FROM nabidka_item
        ...
        if ($row['ni_id'] && $row['ni_id'] != $id) {
            //If there is a conflicting nabidka
